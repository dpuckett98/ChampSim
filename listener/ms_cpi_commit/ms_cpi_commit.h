...
#ifndef MS_CPI_COMMIT_H
#define MS_CPI_COMMIT_H
...
class ms_cpi_commit : public EventListener {
  
  double WIDTH = 8; // todo: calculate this automatically
  ...
  void print_counters() {
    ...
    fmt::print("ms_cpi_commit interval {} uncounted_blamed {}\n", last_interval_num, uncounted_blamed);*/
  }
  ...
  void process_event(event eventType, void* data) {
    ...
    if (eventType == event::CACHE_TRY_HIT) {
      ...
      double f = std::distance(r_data->begin, r_data->end);
      base_comp += f;
      double omf = WIDTH - f;
      
      if (omf > 0) {
        // ROB drained because ROB is empty
        bool fe_drained = std::empty(o3_cpu->ROB);
        
        if (fe_drained) {
          bool i_cache_miss = false;
          for (auto cm : cache_misses) {
            if (cm.first == last_retired_instr.instr_id + 1 && cm.second == "cpu0_L1I") {
              i_cache_miss = true;
            }
          }
          bool bp_miss = last_retired_instr.branch_mispredicted; // check if the youngest retired instruction had a BP miss
          if (i_cache_miss) {
            icache_comp += omf;
          } else if (bp_miss) {
            bp_comp += omf;
          ...
          // find and blame the instruction causing the stall (oldest instruction in ROB)
          uint64_t blamed_instr = o3_cpu->ROB.front().instr_id;
          
          // associate time with the blamed instruction (it'll be added to the given components when it retires)
          if (blamed_instrs.count(blamed_instr) > 0) {
