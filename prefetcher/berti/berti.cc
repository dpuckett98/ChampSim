...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, 
    uint8_t cache_hit, bool useful_prefetch, uint8_t type, uint32_t metadata_in)
{
// djimenez
if (type == (uint8_t) access_type::PREFETCH) return metadata_in; // there are instruction prefetches with my stuff
  ...
  if constexpr (champsim::debug_print) 
  {
    std::cout << "[BERTI] operate cpu: " << cpu;
  ...
  if (!cache_hit) // This is a miss
  {
    ...
    latencyt[cpu]->add(line_addr, ip, false, current_time.time_since_epoch() / clock_period); // Add @ to latency
    historyt[cpu]->add(ip, line_addr, current_time.time_since_epoch() / clock_period); // Add to the table
  } else if (cache_hit && scache[cpu]->is_pf(line_addr)) // Hit bc prefetch
  {
    ...
    berti[cpu]->find_and_update(latency, ip, current_time.time_since_epoch() / clock_period & TIME_MASK, line_addr, cpu);

    historyt[cpu]->add(ip, line_addr, current_time.time_since_epoch() / clock_period & TIME_MASK);
  } else
  {
    if constexpr (champsim::debug_print) 
...
// std::cout << metadata_in << std::endl;
    if (prefetch_line(p_addr, fill_this_level, metadata_in))
    {
      ...
      if (fill_this_level)
      {
        ...
        {
          ++num_load;
          latencyt[cpu]->add(p_b_addr, ip, true, current_time.time_since_epoch() / clock_period);
        }
      }
    }
  }
    ...
    uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in)
{
  ...
  if (cycle != 0 && ((current_time.time_since_epoch() / clock_period & TIME_MASK) > cycle))
    latency = (current_time.time_since_epoch() / clock_period & TIME_MASK) - cycle;
